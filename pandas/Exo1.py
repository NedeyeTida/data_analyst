...
print("Nombre de professeurs par rang")
print(df.groupby('rank').size())

print("Salaire moyen par rang")
print(df.groupby("rank")['salary'].mean().rename("Salaire moyen").reset_index())
#df.groupby('rank')["salary"].mean() : retourne une serie, rank devient l'index de la sortie 